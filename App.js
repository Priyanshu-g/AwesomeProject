...
export default class App extends Component {
  ...
  NewCodes() {
    let newCodeName = this.state.newCodeName;
    let newCodeCode = this.state.newCodeCode;
    let newCodeURL = this.state.newCodeURL;
    let { dataSource } = this.state;
    let { myCodes } = this.state;
    if (newCodeName != "" && newCodeCode != "" && newCodeURL != "") {
      this.setState({ newCodeName: "", newCodeCode: "", newCodeURL: "" });
      let newCode = {
        name: newCodeName,
        slug: newCodeCode,
        urlAbsolute: newCodeURL,
      };
      dataSource.push(newCode);
      myCodes.push(newCode);
    }
    if (myCodes.length != 0) {
      return myCodes.map((val, key) => {
        const iconUrl = val.icon;
        if (iconUrl != null) {
          let image = { uri: iconUrl };
          return (
      ...
      return (
        <View style={styles.background}>
          ...
          <Icon
            ...
            type="material"
            onPress={() =>
              this.setState({ makeCode: true, myAccountPage: false })
            }
          />
          <View style={styles.cell}>
            <Text style={styles.text}>My Codes</Text>
          </View>
          <ScrollView>
            <View>{this.NewCodes()}</View>
          </ScrollView>
        </View>
      );
    }
      ...
      return (
        <View style={styles.background}>
          <Icon
            ...
            type="material"
            onPress={() =>
              this.setState({ myAccountPage: true, makeCode: false })
            }
          />
          <View style={styles.inputbox}>
            ...
            <Input
              inputStyle={styles.text}
              onChangeText={(newCodeName) => {
                this.setState({ newCodeName });
              }}
            ></Input>
            <Text style={styles.text}>Code: </Text>
            <Input
              inputStyle={styles.text}
              onChangeText={(newCodeCode) => {
                this.setState({ newCodeCode });
              }}
            ></Input>
            <Text style={styles.text}>URL: </Text>
            <Input
              inputStyle={styles.text}
              onChangeText={(newCodeURL) => {
                this.setState({ newCodeURL });
              }}
            ></Input>
            <Icon
              style={styles.accountButton}
